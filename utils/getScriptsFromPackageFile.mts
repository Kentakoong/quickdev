...
interface PackageJsonReturnProps {
  name: string;
  description: string;
  scripts: string;
}

export default async function getScriptsFromPackageFile(
  projectPath: string,
  checkUndefined: boolean = true
): Promise<PackageJsonReturnProps> {
  const packageJsonPath = `${projectPath}/package.json`;
  const dirAvailable = checkDirectory(packageJsonPath, "file");
  if (!dirAvailable) throw new Error("Project not found");

  const jsonString = fs.readFileSync(packageJsonPath).toString();

  if (jsonString == "{}") throw new Error("package.json is empty");

  const parsedPackageJson = JSON.parse(jsonString);

  var name: string = parsedPackageJson.name;
  var description: string = parsedPackageJson.description;
  ...
  if (scripts != undefined) {
    ...
    const scriptSelection: string = await select({
