...
export default async function loadProfiles(selection) {
    const savedData = getSavedData();
    const selectedWorkspace = savedData.savedData.find((data) => data.value == selection);
    ...
    if (!checkDirectory(selectedWorkspace.projectPath, "dir")) {
        ...
        fs.writeFileSync(savedFileDir, JSON.stringify({
            savedData: savedData.savedData.filter((data) => data.value != selection),
        }));
    }
    else {
        console.log(chalk.cyan(`\n🖥️  We'll be opening project ${selectedWorkspace.name} Project for you!\n`));
        const isModulesInstalled = checkDirectory(`${selectedWorkspace.projectPath}/node_modules`, "dir");
        const packageManager = async () => {
            if (checkDirectory(`${selectedWorkspace.projectPath}/yarn.lock`, "file"))
                return "yarn";
            else if (checkDirectory(`${selectedWorkspace.projectPath}/pnpm-lock.yaml`, "file"))
                return "pnpm";
            else if (checkDirectory(`${selectedWorkspace.projectPath}/package-lock.json`, "file"))
                return "npm";
            else {
        ...
        if (!isModulesInstalled) {
            ...
            await execWaitForOutput(`echo 'Installing Dependencies' && cd ${selectedWorkspace.projectPath} &&
            ${selectedWorkspace.packageManager} install
            `);
        }
        const cd = `cd ${selectedWorkspace.projectPath}`;
        const packageManagerStart = `${selectedWorkspace.packageManager}${selectedWorkspace.packageManager == "npm" ? " run" : ""} ${selectedWorkspace.scripts}`;
        const start = `"${cd}\ 
${packageManagerStart}"`;
