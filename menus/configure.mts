...
export default async function configurePane() {
  ...
  if (settingsMenuSelection == "back") return;

  const fieldToEdit = savedData.savedData.find(
    (data) => data.value == settingsMenuSelection
  );

  if (!fieldToEdit) throw new Error("Field not found");
  ...
  if (newValue == "back") return;

  var newName: string | undefined;
  var newDescription: string | undefined;
  var newScripts: string | undefined;
  ...
  if (newValue == "scripts") {
    const packageJSONData = await getScriptsFromPackageFile(
      fieldToEdit.projectPath,
      false
    ).catch(() => {
  ...
  const newSavedData = savedData.savedData.map((data) => {
    if (data.value == fieldToEdit.value) {
      ...
      if (newScripts) data.scripts = newScripts;
    }
    return data;
  });
